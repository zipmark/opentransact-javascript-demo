...
function setupAddressForm() {
  ...
  window.addEventListener("storeUpdated", function (event) {
    ...
    updateAccountSelect(window.store.profile.accounts);
  });
}

function setupInfoPanels() {
  let showTransactionsLink = document.getElementById("showTransactionsLink");
  let showActivitiesLink = document.getElementById("showActivitiesLink");

  let activeClasses =
    "border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-5 border-b-2 text-lg leading-6 font-medium";
  let inactiveClasses =
    "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-5 border-b-2 text-lg leading-6 font-medium";
  if (showTransactionsLink && showActivitiesLink) {
    showTransactionsLink.addEventListener(
      "click",
      () => {
        hideElement("activitiesList");
        showElement("transactionsList");
        showTransactionsLink.className = activeClasses;
        showActivitiesLink.className = inactiveClasses;
      },
      false
    );

    showActivitiesLink.addEventListener(
      "click",
      () => {
        hideElement("transactionsList");
        showElement("activitiesList");
        showTransactionsLink.className = inactiveClasses;
        showActivitiesLink.className = activeClasses;
      },
      false
    );
  }
...
function updateAccountSelect(accounts) {
  let template = `<option value="{{id}}">{{nickname}}</option>`;
  let select = document.getElementById("accountSelect");
  let options = accounts.map((account) => Mustache.render(template, account));
  select.innerHTML = options.join("\n");
}

function initCurrencySelect() {
  let template = `<option value="{{currency}}">{{currency}}</option>`;
  let select = document.getElementById("currencySelect");
  let options = Object.keys(currencyCodes).map((currency) =>
    Mustache.render(template, { currency: currency })
  );
  select.innerHTML = options.join("\n");
}
